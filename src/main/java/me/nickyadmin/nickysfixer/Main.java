...
public final class Main extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        pm.registerEvents(new antiIllegalOnMove(this), this);
            ...
            if (label.equalsIgnoreCase("nickysfixer") || label.equalsIgnoreCase("nf") || label.equalsIgnoreCase("n") || label.equalsIgnoreCase("nickysfixes")) {
                if (args.length == 0) {
                    if (!(sender instanceof Player)) {
                        sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&4[NickysFixer] &3this server runs NickysFixer 0.6.95 ALPHA by &6NickyAdmin&3#6965"));
                        return true;
                    }
                }
                ...
                if (args[0].equalsIgnoreCase("tps")) {
                    if (!(sender instanceof Player)) {
                        if (!(GetTps_1_12_R1.getTps() < 20)) {
                            ...
                            sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&4[Nickysfixer] &3" + GetTps_1_12_R1.getTps()));
                        }
                    }
                }
            }
            if (label.equalsIgnoreCase("server")) {
                if (!(sender instanceof Player)) {
                    if (getConfig().getBoolean("server-command")) {
                        sender.sendMessage(ChatColor.GREEN + "this server is running " + Bukkit.getServer().getName() + ", " + Bukkit.getServer().getBukkitVersion() + ", [NickysFixer]");
                    } else {
                        sender.sendMessage(ChatColor.translateAlternateColorCodes('&', "&4[NickysFixer] &3" + getConfig().getString("command-disabled-message")));
                    }
                }
            }
